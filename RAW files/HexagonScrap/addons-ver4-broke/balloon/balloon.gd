...
@export var ratio: float = 1.0: set = _set_ratio
@export var font_height_adjust: int = 0: set = _set_font_height_adjust
@export var padding: int = 8: set = _set_padding
@export var shadown_width: int = 4: set = _set_shadown_width
@export var text_color: Color = Color(0,0,0,1): set = _set_text_color
@export var color: Color = Color(1,1,1,1): set = _set_color
@export var color_center: Color = Color(1,1,1,1): set = _set_color_center
@export var color_shadow: Color = Color(0,0,0,1): set = _set_color_shadow
@export var arrow_width: float = 0.25: set = _set_arrow_width
@export var normal_font: Font = preload("res://addons/balloon/assets/OpenComicFont.tres"): set = _set_font
#export(Font) var bold_font
#export(Font) var italics_font
#export(Font) var bold_italics_font
#export(Font) var mono_font
@export var lock_target: NodePath = null: set = _set_target
@export var words_per_minute: int = 200 # world median words readed by minute
@export var typewriter:bool = false
@export var auto_hide: bool = true
@export var bubble_effect: bool = false: set = _set_bubble_effect
@export var balloon_material: Material = null: set = _set_balloon_material
@export var text_material: Material = null: set = _set_text_material
@export var script_path: NodePath = null: set = _set_script_path
@export var ui_theme: Theme = preload("res://addons/balloon/assets/balloon.theme"): set = _set_ui_theme
...
func _set_text(txt):
	...
	if not txt:
		txt = ""
	text = txt
	if txt == "" and not Engine.is_editor_hint():
		hide()
	else:
		render_text(txt)
		update()
...
func ask(txt, param1 = null, param2 = null, param3 = null, param4 = null):
	...
	if not param1:
		...
		yes.connect("pressed", Callable(self, "_button_true_clicked"))
		var no = Button.new()
		no.text = tr('No')
		no.size_flags_horizontal = SIZE_FILL | SIZE_EXPAND
		no.connect("pressed", Callable(self, "_button_false_clicked"))
		_panel.add_child(no)
		...
		for resp in param1:
			...
			r.text = tr(resp)
			r.connect("pressed", Callable(self, "_button_resp_clicked").bind(index))
			_panel.add_child(r)
			if pary_functions:
	...
	_panel.custom_minimum_size = Vector2(size,_panel.size.y)
...
func _draw():
	...
	if Engine.is_editor_hint():
		draw_texture( preload("res://addons/balloon/assets/icon_balloon.png"), Vector2(-8,-8) + size*0.5, Color(1,1,1,1) )
		if text=="":
			var cr = Color("#a5efac")
			if arrow_target and vertices.size():
				draw_line( Vector2(-3,7) + size*0.5, _arrow_target, Color("#a5efac"), 1 )
			return
	...
	var left_top = global_position #- extra_offset
	var right_bottom = global_position + extra_offset*2.0
	var computed_size = Vector2(right_bottom.x-left_top.x,right_bottom.y-left_top.y)
	if _panel and _panel.get_child_count()>0:
		computed_size.x = max(computed_size.x,_panel.size.x)
		computed_size.y = computed_size.y + _panel.size.y + padding
	_offset = Vector2()
	...
	if _panel and _panel.get_child_count()>0:
		_panel.position = Vector2(int(((_panel.size.x*-0.5 + rad*_ratio.x)+_offset.x)*_bubble_extra.x),int(((rad*_ratio.y*2.0 + padding*2 + shadown_width)+_offset.y)*_bubble_extra.y) )
	
	if _surface1:
		if Engine.is_editor_hint() and _surface1.has_method('ex_update'):
...
func _ready():
	if not normal_font:
		...
		self.normal_font = super.get_font('font')
	#if not bold_font:
	...
	if Engine.is_editor_hint():
		...
		_surface1.connect("draw", Callable(_surface1, "ex_update").bind(self))
		_surface2.connect("draw", Callable(_surface2, "ex_update").bind(self))
	add_child(_surface1)
	add_child(_surface2)
